...
preference_select() {
  ...
  if [[ $CHOICE != "NONE" ]]; then
    execute_command paru -Sy
    execute_command paru -S --needed $CHOICE
...
install_paru() {
  ...
  execute_command git clone https://aur.archlinux.org/paru.git /tmp/paru
  cd /tmp/paru
...
install_icon_theme() {
  ...
  if [[ $CHOICE == "Tela Icon Theme" ]]; then
    ...
    paru -S --noconfirm --needed papirus-icon-theme papirus-folders
    ...
    paru -S --noconfirm --needed adwaita-icon-theme
